...
function fillColor() {
  ...
  sliderTrack.style.background = `linear-gradient(to right, #dadae5 ${percent1}% , #4295E4 ${percent1}% , #4295E4 ${percent2}%, #dadae5 ${percent2}%)`;
}
...
for (const elems of categoryDivFlex) {
  elems.addEventListener("click", (e) => {
    let newProduct = products.filter(
      (item) => item.category == e.target.innerHTML
    );
